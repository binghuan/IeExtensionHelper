...
CClassFactory::CClassFactory(CLSID clsid, HINSTANCE instance) : CUnknown<IClassFactory>(SupportedIIDs,2)
{
	m_clsid = clsid;
	m_ManifestParser = new IeManifestParser(instance);
	m_IeExtBHOInfo = m_ManifestParser->getIeExtBHOInfo();
	m_IeExtToolbarButtonInfo = m_ManifestParser->getIeExtToolbarButtonInfo();
	m_IeExtContentScriptInfo = m_ManifestParser->getIeExtContentScriptInfo();
}
...
// This is called to lock/unlock our DLL in memory by the host process. 
// While the DLL is locked, it will not be unloaded from memory.
